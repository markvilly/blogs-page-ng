...
@Component({
    selector: 'app-blog',
    template: `
        <div class="relative w-75 border border-gray-300 rounded-lg shadow-md mb-6">
            <div class="absolute bg-gray-200 h-[250px] w-full rounded-t-lg">
                <div class="absolute top-4 right-4">
                    <div class="bg-black text-white opacity-75 rounded-full px-4 text-sm py-2">
                        {{ filterName }}
                    </div>
                </div>
            </div>
            <div class="mt-60 p-4">
                ...
                <button (click)="onSelectBlog()" class="mt-4 text-blue-500 py-2 rounded hover:text-blue-600 hover:underline hover:cursor-pointer">
                    Read More
                </button>
            </div>
        </div>
